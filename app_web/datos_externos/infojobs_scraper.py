...
def scrape_infojobs(titulo='', ubicacion=''):
    """
    Scrapes job offers from InfoJobs based on title and location.
    ...
    try:
        ...
        ofertas = []
        job_listings = soup.find_all('div', class_='ij-OfferCard')
        
        for job in job_listings:
            try:
                # Extract job details
                titulo = job.find('h2', class_='ij-OfferCardContent-description-title').text.strip()
                empresa = job.find('span', class_='ij-OfferCardContent-description-company').text.strip()
                ubicacion = job.find('span', class_='ij-OfferCardContent-description-location').text.strip()
                fecha_text = job.find('span', class_='ij-OfferCardContent-description-date').text.strip()
                ...
                descripcion = detail_soup.find('div', class_='ij-OfferCardContent-description').text
                ...
                habilidades = []
                skills_section = detail_soup.find('div', class_='ij-OfferCardContent-description-skills')
                if skills_section:
        ...
        print(f"Error al hacer scraping de InfoJobs: {str(e)}")
        return []
